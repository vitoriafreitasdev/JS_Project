...
const exerciseId = split[1]

editExercise.addEventListener("click", (e) => {
    e.preventDefault()
    ...
    if(exerciseName.value.length == 0 || exerciseSeries.value == 0 || exerciseReps.value.length == 0 || exerciseWeight.value.length == 0 || exerciseDay.value.length == 0){
        ...
        setTimeout(function(){
           ...
           const exercise = {
            ...
            webFetch(`/user/edit/${exerciseId}`, {
                method: "PUT",
                body: JSON.stringify(exercise)
            })
            .then((data) => window.location.assign(`/frontend/userPage/userPage.html?id=${data.userUptade._id}`))
            .catch((error) => {
